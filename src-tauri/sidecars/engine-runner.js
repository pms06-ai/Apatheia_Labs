...
function saveFindings(config, caseId, findings) {
  try {
    ...
    const stmt = database.prepare(`
      INSERT INTO findings (id, case_id, engine, title, description, finding_type, severity, document_ids, entity_ids, regulatory_targets, evidence, metadata, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, '[]', '[]', ?, '{}', datetime('now'))
    `);
...
async function executeRequest(config, request) {
  ...
  try {
    ...
    log(`Executing engine: ${request.engine_id} (normalized: ${normalizedId})`);
    ...
    if (findings.length > 0) {
      try {
        ...
        log(`Warning: Failed to save findings: ${e}`);
      }
    }
    log(`Engine ${request.engine_id} completed with ${findings.length} findings`);
    return {
      success: true,
      engine_id: request.engine_id,
      findings,
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    log(`Engine ${request.engine_id} failed: ${errorMessage}`);
    return {
      success: false,
      engine_id: request.engine_id,
      error: errorMessage,
