...
function getDatabase(config) {
    if (!readDb) {
        ...
        readDb = new better_sqlite3_1.default(config.database_path, { readonly: true });
...
function getWriteDatabase(config) {
    if (!writeDb) {
        ...
        writeDb = new better_sqlite3_1.default(config.database_path);
    ...
    `);
        ...
        const insertMany = database.transaction((items) => {
            for (const finding of items) {
                stmt.run(finding.id, caseId, finding.engine_id, finding.title, finding.description, finding.finding_type, finding.severity, finding.confidence, JSON.stringify(finding.document_ids), JSON.stringify(finding.evidence));
            }
        });
        insertMany(findings);
...
function getAI(config) {
    if (!anthropic) {
        ...
        anthropic = new sdk_1.default({ apiKey: config.anthropic_api_key });
    }
    return anthropic;
}
/**
 ...
 */
async function callWithRetry(fn, maxRetries = 3, baseDelayMs = 1000, maxDelayMs = 10000) {
    let lastError = null;
...
async function generateJSON(config, systemPrompt, userContent) {
    ...
    if (config.use_claude_code) {
        ...
        const result = await (0, claude_code_client_1.executeJSONPrompt)(systemPrompt, userContent, { timeout: 180000 });
        if (!result.success) {
 ...
 */
function sanitizeForPrompt(content, maxLength = 50000) {
    if (!content)
        return '';
    return content
...
const contradictionEngine = async (config, caseId, documentIds) => {
    ...
    const formattedDocs = docContents.map(d => `=== DOCUMENT: ${d.name} (ID: ${d.id}, Type: ${d.type || 'unknown'}) ===\n${d.content}`).join('\n\n~~~\n\n');
...
// Omission Engine
const omissionEngine = async (config, caseId, documentIds) => {
...
const expertWitnessEngine = async (config, caseId, documentIds) => {
    const docs = loadDocuments(config, documentIds);
...
// Narrative Engine
const narrativeEngine = async (config, caseId, documentIds) => {
    const docs = loadDocuments(config, documentIds);
    if (docs.length < 2) {
        return [];
    }
...
// Coordination Engine
const coordinationEngine = async (config, caseId, documentIds) => {
    const docs = loadDocuments(config, documentIds);
    if (docs.length < 2) {
        return [];
    }
...
const entityResolutionEngine = async (config, caseId, documentIds) => {
    const docs = loadDocuments(config, documentIds);
    const allContent = docs.map(doc => sanitizeForPrompt(loadDocumentContent(config, doc.id), 40000)).join('\n\n~~~\n\n');
  ...
  `);
    ...
    const insertEntities = database.transaction((entities) => {
        for (const entity of entities || []) {
            ...
            stmt.run(id, caseId, entity.canonicalName, entity.entityType, JSON.stringify(entity.aliases), entity.role || null, entity.institution || null, entity.professionalRegistration || null);
        }
    });
...
// Temporal Engine
const temporalParserEngine = async (config, caseId, documentIds) => {
    const docs = loadDocuments(config, documentIds);
    const docContents = docs.map(doc => ({
        id: doc.id,
        name: sanitizeFilename(doc.filename),
  ...
  `);
    ...
    const insertEvents = database.transaction((events) => {
        for (const [idx, event] of (events || []).entries()) {
            eventStmt.run(`event-${caseId.slice(0, 8)}-${idx}`, caseId, event.date, event.time || null, event.precision, event.description, event.eventType || null, event.sourceDocumentId || null, event.sourcePage || null);
        }
    });
...
// =============================================================================
const documentaryEngine = async (config, caseId, documentIds) => {
    const findings = [];
    const docs = loadDocuments(config, documentIds);
    for (const doc of docs) {
        // Heuristic: Look for editorial patterns in document names or content
        const hasEditorialIndicators = doc.filename.toLowerCase().includes('draft') ||
...
const argumentationEngine = async (config, caseId, documentIds) => {
    const findings = [];
    const docs = loadDocuments(config, documentIds);
    for (const doc of docs) {
        if (!doc.extracted_text)
        ...
        const hasArgumentIndicators = text.includes('therefore') ||
...
const biasDetectionEngine = async (config, caseId, documentIds) => {
    const findings = [];
    const docs = loadDocuments(config, documentIds);
    for (const doc of docs) {
        if (!doc.extracted_text)
...
const accountabilityAuditEngine = async (config, caseId, documentIds) => {
    const findings = [];
    const docs = loadDocuments(config, documentIds);
    for (const doc of docs) {
        if (!doc.extracted_text)
...
const professionalTrackerEngine = async (config, caseId, documentIds) => {
    const findings = [];
    ...
    const professionalMentions = new Map();
...
// =============================================================================
const promptExecutorEngine = async (config, caseId, documentIds, options) => {
...
async function main() {
    ...
    if (config.use_claude_code) {
        const ccStatus = (0, claude_code_client_1.checkClaudeCodeStatus)();
